...
//INF 117 Start
if (isset($_REQUEST['beginDate']) && !empty($_REQUEST['beginDate']))
{
...
function updateVolunteer($exists) {
	...
	*/
	...
	if ($exists) { // volunteer exists so just update
		$sql = "Update volunteers Set first_name='$firstName', middle_name='$middleName',last_name='$lastName', organization='$organization', phone='$phone', email='$email', active_id='$active_id', street='$street', city='$city', state='$state',zip='$zip', notes='$notes', signed_up='$signed_up' where id=$id";						
		$r = $db->q($sql);
		getError($r);
...
function updateTree($exist){
	...
	if($exist){
		$sql = "Update grower_trees Set grower_id='$grower_id', tree_type='$tree_type_id', varietal='$varietal', number='$number',  avgHeight_id='$avgHeight_id',  chemicaled='$chemicaled_id', lastdate='$last_date' where id=$id";
		$r = $db->q($sql);	
		if (getError($r))
			return;
		for ($i=1; $i<14 ; $i++) {
...
*/
	case 'get_totals':
				...
				(
					...
					from harvests join events on harvests.event_id = events.id
					WHERE events.date >= '.$beginDate.' and events.date <= '.$endDate.'
				)as a
				join
				(
					select sum(hour)as hours
					from volunteer_events join events on volunteer_events.event_id = events.id
					WHERE events.date >= '.$beginDate.' and events.date <= '.$endDate.'
				) as b
				join
				(
					...
					from harvests join events on harvests.event_id = events.id
					WHERE events.date >= '.$beginDate.' and events.date <= '.$endDate.'
					AND harvests.number is not null
				)as c
				join
				(
					select sum(pound) as delivered
					from drivings join events on drivings.event_id = events.id
					WHERE events.date >= '.$beginDate.' and events.date <= '.$endDate.'
				)as d
				...
				left join
					(	select y.city, z.count
						...
						AND events.date >= '.$beginDate.' and events.date <= '.$endDate.'
				...
				(
					...
					FROM `events` join `drivings` on drivings.event_id = events.id join `distributions` on drivings.distribution_id = distributions.id
					AND events.date >= '.$beginDate.' and events.date <= '.$endDate.'
...
//INF 117 End
	...
	case 'get_trees': // same priv as grower
		...
		$sql = "SELECT gt.id AS tree_id, Concat(g.first_name,' ', g.last_name) AS Owner, g.id AS grower_id , tt.id AS 'tree_type_id', tt.name AS 'Tree type', gt.varietal AS Varietal, gt.number AS Number, gt.chemicaled AS Chemicaled_id, IF((gt.chemicaled=0),'No','Yes') AS 'Chemicals Used_id', th.id AS avgHeight_id, th.name AS Height, 
					(	SELECT group_concat(mh.month_id)
						FROM	month_harvests mh
						WHERE mh.tree_id = gt.id ) month,
